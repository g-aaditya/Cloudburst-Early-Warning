...
with st.sidebar:
    ...
    if fexists:
        mtime_str = datetime.fromtimestamp(current_mtime).strftime(
            "%d %b %Y, %H:%M:%S") if current_mtime else "—"
        st.markdown(f"""
    ...
    if st.session_state.get("auto_refresh"):
        st.markdown(
            "<div style='font-size:10px;color:#00d4aa;font-family:monospace'>🟢 Auto-detect ON — checks every 5 sec</div>", unsafe_allow_html=True)
    else:
        st.markdown(
            "<div style='font-size:10px;color:#4a6080;font-family:monospace'>⚫ Auto-detect OFF — use Reload button</div>", unsafe_allow_html=True)
    st.markdown(
        f"<div style='font-size:10px;color:#4a6080;font-family:monospace;margin-top:4px'>File: <b>{fname}</b></div>", unsafe_allow_html=True)

    # ── Trigger periodic check when auto is ON (uses st_autorefresh via meta refresh) ──
    if st.session_state.get("auto_refresh"):
        st.markdown(
            """<script>setTimeout(function(){ window.location.reload(); }, 8000);</script>""", unsafe_allow_html=True)
...
c1.metric("Cloudburst Probability",
          f"{result.cloudburst_probability:.0f}%", result.risk_level)
...
c4.metric("Flash Flood Risk",
          f"{result.flash_flood_probability:.0f}%", f"Runoff: {result.peak_runoff:.0f} m³/s")
c5.metric("Landslide Risk",
          f"{result.landslide_probability:.0f}%", f"Slope: {inp.slope}°")
c6.metric("Affected Population",
          f"{result.affected_population:,}", "Estimated")
...
with tab1:
    ...
    with c_left:
        st.markdown(
            '<div class="section-title">Probability Timeline (ConvLSTM Output)</div>', unsafe_allow_html=True)
        tlabels = ['T-6h', 'T-5h', 'T-4h', 'T-3h',
                   'T-2h', 'T-1h', 'Now', '+1h', '+2h', '+3h']
        base = result.cloudburst_probability / 100
        prob_seq = [round(min(99, max(1, base*f*100)), 1)
                    for f in [.08, .14, .22, .33, .48, .68, 1.0, 1.08, .92, .72]]
        fig = go.Figure()
        ...
        fig.update_layout(**PLOTLY_LAYOUT, **PLOTLY_AXES,
                          height=250, yaxis_range=[0, 100], showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

    with c_right:
        st.markdown(
            '<div class="section-title">Cloud Activity — ConvLSTM Spatial-Temporal</div>', unsafe_allow_html=True)
        cloud_data = [
            round(min(99, max(1, p*(0.85+np.random.uniform(-.1, .1))))) for p in prob_seq]
        fig2 = go.Figure(go.Bar(x=tlabels, y=cloud_data,
                                marker_color=[risk_color(d) for d in cloud_data], marker_line_width=0))
        fig2.update_layout(**PLOTLY_LAYOUT, **PLOTLY_AXES,
                           height=250, yaxis_range=[0, 100], showlegend=False)
    ...
    with ci1:
        ...
        for name, val, col in [
            ...
            ("Precipitable water",    min(
                99, round(result.moisture_index*.9)), '#c77dff'),
        ]:
            st.markdown(
                f"<div style='font-size:12px;color:#7a9bc8;margin-bottom:2px'>{name} — <b style='color:#e8f0ff'>{val}%</b></div>", unsafe_allow_html=True)
            st.progress(int(val))
    ...
    with ci3:
        ...
        for k, v in [
            ...
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:13px;padding:4px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:#e8f0ff;font-weight:500'>{v}</span></div>", unsafe_allow_html=True)
        ...
        fig.update_layout(
            ...
            **PLOTLY_LAYOUT, height=280,
            ...
            yaxis=dict(showgrid=False, tickfont=dict(size=9),
                       autorange='reversed', color='#4a6080')
        )
        return fig
...
with tab3:
    ...
    with i1:
        st.markdown(
            '<div class="section-title">Hydrological Impact</div>', unsafe_allow_html=True)
        for k, v in [
            ("Flash Flood Probability",
             f"{result.flash_flood_probability:.0f}%"),
            ...
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:13px;padding:6px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:#e8f0ff;font-weight:500'>{v}</span></div>", unsafe_allow_html=True)

    with i2:
        st.markdown(
            '<div class="section-title">Population & Infrastructure</div>', unsafe_allow_html=True)
        for k, v in [
            ("Affected Population",     f"{result.affected_population:,}"),
            ("Road Infrastructure Risk", result.road_risk_level),
            ("Agricultural Land (ha)",
             f"{result.agricultural_area_affected:.1f}"),
        ]:
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:13px;padding:6px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:#e8f0ff;font-weight:500'>{v}</span></div>", unsafe_allow_html=True)
    ...
    with iz1:
        st.markdown(
            '<div class="section-title">Landslide Risk Zones</div>', unsafe_allow_html=True)
        p_val = result.cloudburst_probability
        for name, val, desc in [
            ("⛰ Steep Slopes",    min(99, round(p_val*.85+inp.slope/2)),
             "High debris flow potential"),
            ("🏞 Valley Floors",   min(99, round(p_val*.70)),
             "Flash flood accumulation"),
            ("🌄 Debris Channels", min(
                99, round(p_val*.90+inp.slope/3)), "Rapid mass movement"),
    ...
    with iz2:
        st.markdown(
            '<div class="section-title">Multi-Hazard Risk</div>', unsafe_allow_html=True)
        p_val = result.cloudburst_probability
        fig_d = go.Figure(go.Pie(
            labels=['Flash Flood', 'Landslide',
                    'Infrastructure', 'Agri Loss', 'Population'],
    ...
    st.markdown('<div class="section-title">6-Hour Risk Forecast</div>',
                unsafe_allow_html=True)
...
with tab4:
    ...
    with ph1:
        st.markdown(
            '<div class="section-title">PINN Physics Indicators</div>', unsafe_allow_html=True)
        phys = result.physics
        for name, val, col in [
            ...
            st.markdown(
                f"<div style='font-size:12px;color:#7a9bc8;margin:8px 0 2px'>{name} — <b style='color:#e8f0ff'>{val:.1f}%</b></div>", unsafe_allow_html=True)
            st.progress(int(min(100, val)))

    with ph2:
        st.markdown('<div class="section-title">Physics Radar</div>',
                    unsafe_allow_html=True)
        cats = ['Moisture', 'Instability', 'Orographic',
                'Convection', 'Soil', 'Pressure Drop']
    ...
    with pv1:
        st.markdown('<div class="section-title">Derived Values</div>',
                    unsafe_allow_html=True)
        for k, v in [
            ...
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:13px;padding:5px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:#e8f0ff;font-weight:500'>{v}</span></div>", unsafe_allow_html=True)

    with pv2:
        st.markdown(
            '<div class="section-title">Terrain-Atmosphere</div>', unsafe_allow_html=True)
        ...
        for k, v in [
            ...
            ("Slope-Moisture Index",
             f"{min(99,round(inp.slope*inp.soil_moisture/60))}%"),
            ("Valley Channeling",     "Active" if inp.elevation >
             800 and inp.slope > 10 else "Weak"),
    ...
    with pv3:
        st.markdown(
            '<div class="section-title">PINN Consistency</div>', unsafe_allow_html=True)
        for k, v in [
            ("Law Adherence",    f"{phys.pinn_law_adherence:.1f}%"),
            ("Plausibility",     f"{phys.pinn_plausibility:.1f}%"),
            ("Generalization",   "82%"),
            ("Rare Event Mode",  "High" if result.cloudburst_probability >
             70 else "Standard"),
        ]:
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:13px;padding:5px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:#e8f0ff;font-weight:500'>{v}</span></div>", unsafe_allow_html=True)
...
with tab5:
    ...
    with xe1:
        st.markdown(
            '<div class="section-title">Top Contributing Factors (SHAP-style XAI)</div>', unsafe_allow_html=True)
    ...
    with xe2:
        st.markdown(
            '<div class="section-title">Feature Importance</div>', unsafe_allow_html=True)
        xai_sorted = sorted(
    ...
    with ma1:
        ...
        for name, val, col in [
            ...
            st.markdown(
                f"<div style='font-size:12px;color:#7a9bc8;margin:6px 0 2px'>{name} — <b style='color:#e8f0ff'>{val:.1f}%</b></div>", unsafe_allow_html=True)
            st.progress(int(min(100, val)))
    ...
    with ma3:
        ...
        for k, v, c in [
            ...
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:13px;padding:5px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:{c};font-weight:500'>{v}</span></div>", unsafe_allow_html=True)
...
with tab6:
    ...
    with al1:
        st.markdown('<div class="section-title">Active Alert</div>',
                    unsafe_allow_html=True)
        cls_map = {"EXTREME": "alert-extreme", "HIGH": "alert-high",
                   "WATCH": "alert-watch", "NORMAL": "alert-normal"}
        st.markdown(
            f'<div class="{cls_map.get(result.alert_level,"alert-normal")}"><b>{result.alert_message}</b></div>', unsafe_allow_html=True)
        ...
        st.markdown(
            '<div class="section-title" style="margin-top:16px">Response Recommendations</div>', unsafe_allow_html=True)
        for i, rec in enumerate(result.recommendations, 1):
            st.markdown(
                f"<div style='display:flex;gap:10px;padding:7px 0;border-bottom:1px solid rgba(79,158,255,0.08);font-size:13px'><span style='color:#4f9eff;font-weight:600;min-width:20px'>{i}.</span><span style='color:#c8d8f0'>{rec}</span></div>", unsafe_allow_html=True)

    with al2:
        st.markdown(
            '<div class="section-title">6-Hour Threat Timeline</div>', unsafe_allow_html=True)
        fcast_labels = ['Now', '+1h', '+2h', '+3h', '+4h', '+5h', '+6h']
        ...
        for y0, y1, fill_c, lbl in [(75, 100, "rgba(255,71,87,0.06)", "Extreme"), (55, 75, "rgba(255,154,60,0.06)", "High"), (30, 55, "rgba(255,209,102,0.06)", "Watch")]:
            fig_al.add_hrect(y0=y0, y1=y1, fillcolor=fill_c,
                             line_width=0, annotation_text=lbl)
        fig_al.update_layout(**PLOTLY_LAYOUT, **PLOTLY_AXES,
                             height=240, yaxis_range=[0, 100], showlegend=False)
        st.plotly_chart(fig_al, use_container_width=True)

        st.markdown(
            '<div class="section-title">Data Requirements</div>', unsafe_allow_html=True)
        for k, v, c in [
            ...
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;font-size:12px;padding:5px 0;border-bottom:1px solid rgba(79,158,255,0.08)'><span style='color:#7a9bc8'>{k}</span><span style='color:{c};font-weight:500'>{v}</span></div>", unsafe_allow_html=True)

# ══════════════════════════════════════════════════════════════════════════════
# TAB 7 — EXCEL DATA VIEWER
# ══════════════════════════════════════════════════════════════════════════════
with tab7:
    st.markdown('<div class="section-title">Current Parameters Loaded from Excel</div>',
                unsafe_allow_html=True)
    ...
    with ed1:
        ...
        df_atm = pd.DataFrame(list(atm_data.items()),
    ...
    with ed2:
        ...
        df_ter = pd.DataFrame(list(ter_data.items()),
                              columns=["Parameter", "Value"])
        df_ter["Value"] = df_ter["Value"].astype(str)
        st.dataframe(df_ter, use_container_width=True, hide_index=True)

    st.markdown("---")
    st.markdown('<div class="section-title">Historical Cloudburst Events (from Excel)</div>',
                unsafe_allow_html=True)
    ...
    if hist_df is not None and not hist_df.empty:
        ...
        st.markdown(
            '<div class="section-title" style="margin-top:16px">Historical Data Charts</div>', unsafe_allow_html=True)
        ...
        with hc2:
            try:
                region_counts = hist_df.groupby(
                    ["Region", "Cloudburst(Y/N)"]).size().reset_index(name="count")
                fig_bar = go.Figure()
                for cb_val, col in [("Y", "#ff4757"), ("N", "#4f9eff")]:
                    sub = region_counts[region_counts["Cloudburst(Y/N)"]
                                        == cb_val]
                    if not sub.empty:
                ...
                fig_bar.update_layout(**PLOTLY_LAYOUT, **PLOTLY_AXES, height=260, barmode='group',
                                      title=dict(text="Events by Region", font=dict(
                                          color='#c8d8f0', size=12), x=0),
                                      legend=dict(font=dict(color='#7a9bc8', size=10)))
        ...
        st.info(
            "Historical data not available. Make sure cloudburst_data.xlsx is present.")
...
<div style='text-align:center;font-size:11px;color:#4a6080;padding:8px'>
  "AI-Powered Multimodal Framework for Multi-Hazard Early Warning · Aaditya Goshike & Maddili Benarjee · SRM IST, 2026"
